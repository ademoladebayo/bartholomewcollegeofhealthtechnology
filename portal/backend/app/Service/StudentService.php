...
class StudentService
{
    ...
    public function signIn(Request $request)
    {
        ...
        $student = StudentModel::where('student_id', $request->id)->with('department')->get()->first();
        if ($student == null) {
            return  response(['success' => false, 'message' => "Invalid Student!"]);
        } else {

            if (Hash::check($request->password, $StudentRepository->getPassword($request->id))) {
    ...
    function allFee(Request $request)
    {
        ...
        $class_sector = DepartmentModel::select('class_sector')->where('id', $class)->get()[0]->class_sector;
