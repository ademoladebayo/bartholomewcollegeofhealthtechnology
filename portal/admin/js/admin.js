...
var domain = "http://localhost/smartschoolhub.net/bartholomew";

// LIVE IP
// var ip = "https://smartschoolhub.net/backend/bartholomew";
// var domain = "https://bartholomew.smartschoolhub.net";

// // REMOTE ACCESS
// var ip = "http://192.168.42.168/smartschoolhub.ng/SSHUB_BACKEND/server.php";
// var domain = "http://192.168.42.168/smartschoolhub.ng";

var emailHasBeenVerified = false;
...
function loadSideNav(page) {
  document.getElementById("side_nav").innerHTML = `
    ...
    <li class="nav-item">
        ...
        <span>Lecturer Management</span></a>
    </li>
    ...
    <li class="nav-item">
     ...
     ${
       //(
       //   <li class="nav-item">
       //     <a id="subject" href="subject.html" class="nav-link">
       //       {" "}
...
async function createStudent() {
  ...
  if (
    ...
    if (!emailHasBeenVerified) {
      await sendOTP(student_email);
    }

    if (emailHasBeenVerified) {
      // UPLOAD IMAGE
      await uploadImage(image[0], student_id);

      // PUSH TO API
      warningtoast("<b>Saving Information ... Please wait</b>");
  ...
  swal(
    {
      ...
      inputPlaceholder: "verify your mail",
    },
    function (otp) {
      if (otp === false) return false;
      if (otp === "") {
        swal.showInputError("Enter the OTP you got in your mail!");
        return false;
      } else {
        // PUSH TO API
        warningtoast("<b>Verifying OTP ... Please wait</b>");
        return fetch(ip + "/api/admin/verify-otp", {
          method: "POST",
          headers: {
            Accept: "application/json",
            "Content-type": "application/json",
          ...
          .then((data) => {
            ...
            if (data.success) {
              ...
              swal("VERIFICATION WAS SUCCESSFUL", "", "success");
              setTimeout(function () {
                createStudent();
              }, 1000);
            } else {
...
async function sendOTP(email) {
  var verificationEmail = email;
  // PUSH TO API
  warningtoast("<b>Sending OTP to " + email + " ... Please wait</b>");
  return fetch(ip + "/api/admin/send-otp", {
    method: "POST",
    headers: {
      Accept: "application/json",
      "Content-type": "application/json",
